...
class CameraClient:
    ...
    def get_image(self):
        """sends a request call for the camera api to capture a new image and save it to self.image_path"""
        pass
    ...
    def detect_cube(self, facelets_in_width=11):
        ...
        for contour, hier in zip(contours, hierarchy):
            ...
            if min_area < area < max_area:
                ...
                if abs(inclination) > MAX_ANGLE_INCLINATION_DEG:
                    continue
    ...
    def _classify_color(self, h, s, v):
        ...
        if s < HSV_WHITE_SAT_MAX and v > HSV_WHITE_VAL_MIN:
            return 'W' 
        elif v < HSV_BLACK_VAL_MAX:
            return 'K'  # Black/Shadow
            
        if h < HUE_LIMIT_RED_LOW or h > HUE_LIMIT_RED_HIGH:
            return 'R'  
        elif HUE_LIMIT_RED_LOW <= h < HUE_LIMIT_ORANGE:
            return 'O' 
        elif HUE_LIMIT_ORANGE <= h < HUE_LIMIT_YELLOW:
            return 'Y'  
        elif HUE_LIMIT_YELLOW <= h < HUE_LIMIT_GREEN:
            return 'G'
        elif HUE_LIMIT_GREEN <= h < HUE_LIMIT_BLUE:
            return 'B' 
    ...
    def _preprocess_image_edges(self, image):
        ...
        kernel = np.ones((EDGE_DETECT_KERNEL_SIZE, EDGE_DETECT_KERNEL_SIZE), np.uint8)
        dilated = cv2.dilate(canny, kernel, iterations=DILATION_ITERATIONS)
    ...
    def _calculate_dynamic_area_limits(self, image_width, facelets_in_width=11):
        ...
        """
        offset_factor = EDGE_DETECT_KERNEL_SIZE * (1 + DILATION_ITERATIONS - EROSION_ITERATIONS)
    ...
    def _check_square_geometry(self, vertices):
        ...
        is_square = (edge_variation < SQUARE_SIDE_RATIO_LIMIT) and (diagonal_ratio > RHOMBUS_DIAGONAL_LIMIT)
        return is_square
    ...
    def _filter_facelets_by_size(self, facelets):
        ...
        for f in facelets:
            ...
            if deviation <= MAX_AREA_DEVIATION_LIMIT:
                filtered.append(f)
